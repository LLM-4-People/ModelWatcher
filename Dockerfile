...
EXPOSE 8080
# Production: no --reload (config hot-reload handled by watchfiles on config dir)
CMD ["sh", "-c", "uvicorn backend.main:app --host ${HOST:-0.0.0.0} --port ${PORT:-8080} --loop uvloop --proxy-headers --forwarded-allow-ips \"${FORWARDED_ALLOW_IPS:-127.0.0.1}\" --log-level warning --ws-ping-interval 30 --ws-ping-timeout 90"]
