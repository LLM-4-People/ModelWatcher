...
- [Docker compose setup](#docker-compose-setup) - Step-by-step deployment
  - [1. create a project directory](#1-create-a-project-directory) - Set up config files
  - [2. create environment and config files](#2-create-environment-and-config-files) - Download examples and edit
  - [3. pull and run](#3-pull-and-run) - Start the container
  - [4. check status](#4-check-status) - Monitor startup
- [Volume mounts](#volume-mounts) - Read-write config and data
...
- [Updating](#updating) - Pull latest image and restart
- [Building from source](#building-from-source) - Use compose.modelwatcher.build.yaml for development
...
**Edit `.env`** - add your provider API keys:
...
All runtime values (`PORT`, `restart`, `TZ`, `FORWARDED_ALLOW_IPS`, API keys) come from `.env`, not from inline `environment:` blocks.

## Volume mounts

| Mount | Mode | Purpose |
|-------|------|---------|
| `./config:/app/config:rw` | read-write | Config files. `config.py` strips `reset_epoch: true` from `models.yaml` in-place after processing. Hot-reloaded by `watchfiles` without container restart. |
| `./data:/app/data:rw` | read-write | SQLite database (`metrics.db` + WAL files), VAPID keys, cached favicons. |
...
Config changes in `./config/` are hot-reloaded by `watchfiles` without container restart. Environment variables (`.env`) are read by Docker Compose at container creation time, not at runtime. After editing `.env`, you **must recreate the container**:
...
| `restart` | `unless-stopped` | Docker Compose restart policy. Must be set in `.env` (no fallback in compose file). |
...
Config changes in `./config/` are hot-reloaded by `watchfiles` without container restart. For code updates, pull the latest image:

```bash
docker compose -f compose.modelwatcher.yaml pull
docker compose -f compose.modelwatcher.yaml up -d
```

## Building from source

For local development or customization, use `compose.modelwatcher.build.yaml` instead of the pre-built image. This requires cloning the repository:
